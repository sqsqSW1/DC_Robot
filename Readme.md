...
## V1.2
引入测试框架，通过编译开关管理各模块测试代码，避免 main.c 被测试逻辑污染
...
Test/
├── Inc/
│   ├── test_config.h      # 全局测试开关 (置1开/置0关, 编译器自动裁剪)
│   └── test_runner.h      # 测试调度器声明
└── Src/
    └── test_runner.c      # 测试调度器实现 (统一入口, 按开关调度)
```

**main.c 中只需两行**, 无需出现任何 `#if TEST_XXX`:
```c
#include "test_runner.h"      // 包含测试调度器
Test_Runner();                // 执行所有已开启的模块测试
```

### 编译开关控制 (test_config.h)

| 宏 | 默认值 | 说明 |
|----|--------|------|
| `TEST_MOTOR_ENABLE` | `1` | 电机驱动测试 |
| `TEST_SENSOR_ENABLE` | `0` | 传感器测试（预留） |
| `TEST_PID_ENABLE` | `0` | PID控制测试（预留） |
| `TEST_CHASSIS_ENABLE` | `0` | 底盘运动学测试（预留） |
| `TEST_COMM_ENABLE` | `0` | 通信协议测试（预留） |

**发布前将所有开关置 0 即可** — `#if` 条件为假时编译器和链接器会完全移除测试代码，不占 Flash。

### 新增测试模块步骤

1. `test_config.h` 中添加 `#define TEST_XXX_ENABLE 1`
2. 对应模块文件中实现 `void XXX_Test(void)` 函数
3. `test_runner.c` 中添加 `#if` 块调用 `XXX_Test()`
4. main.c 无需任何修改

### 电机自测流程 (Motor_Test)

测试函数位于 `Motor.c` 末尾，受 `#if TEST_MOTOR_ENABLE` 保护：

| 步骤 | 内容 | 预期结果 |
|------|------|----------|
| Step 1 | 使能电机1 | 电机上电锁定，手动转不动轴 |
| Step 2 | 速度模式正转(CCW) 1秒 → 反转(CW) 1秒 → 停止 | 电机正反交替旋转 |
| Step 3 | 位置模式 3200脉冲正转 → 3200脉冲反转回原位 | 电机往返回到大致原位 |
| Step 4 | 单个电机急停 | 电机立即制动 |
| Step 5 | 电机2+3同步启动 (缓存命令→广播触发) | 两电机同时旋转 (仅多电机环境) |
| 结束 | 失能全部电机 | 电机释放，可自由转动 |

**每步均有超时保护 (1~2秒)，单步失败不阻塞后续测试。** 若电机无响应，用逻辑分析仪检查 PA9(TX) / PA10(RX) 上的 UART 波形。
